...
class Search:
    ...
    #untested!!
    def iterative_deepening(self,cube:Cube) -> tuple[list[tuple[int,int]],int,int] :
        ...
        while solution == None:
            max_depth+=1
            initial_node = Node(cube)
            solution,nodes_expanded,nodes_on_queue = self.__depth_first(initial_node,max_depth)
        sequence = self.__get_solution(solution)
        return sequence,nodes_expanded,nodes_on_queue
    ...
    def __depth_first(self,node:Node, max_depth:int,nodes_expanded:int=0,nodes_on_queue:int=1) -> tuple[Node,int,int]:
        ...
        if node.state.is_solved():
            return node,nodes_expanded,nodes_on_queue-1
        nodes_expanded+=1
        for i in range(0,6):
            direction = i%2
            side = (i//2)
    ...
    def heuristic(self,cube:list[list[list[int]]]) -> float:
